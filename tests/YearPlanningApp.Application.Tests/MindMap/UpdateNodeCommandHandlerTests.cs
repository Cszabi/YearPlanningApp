...
public class UpdateNodeCommandHandlerTests
{
    ...
    private readonly UpdateNodeCommandHandler _handler;

    public UpdateNodeCommandHandlerTests()
    {
        _currentUser.UserId.Returns(_userId);
        _handler = new UpdateNodeCommandHandler(_uow, _currentUser);
    }

    private (Domain.Entities.MindMap map, MindMapNode node) BuildMapWithNode()
    {
        var node = new MindMapNode { Id = Guid.NewGuid(), Label = "Old Label", NodeType = MindMapNodeType.Leaf, PositionX = 0, PositionY = 0 };
        var map = new Domain.Entities.MindMap { Id = Guid.NewGuid(), UserId = _userId, Year = 2026, Nodes = new List<MindMapNode> { node } };
        return (map, node);
    }

    [Fact]
    public async Task Handle_ShouldUpdateLabel_WhenNodeExists()
    {
        ...
        var result = await _handler.Handle(
            new UpdateNodeCommand(2026, node.Id, "New Label", null, null, null, null, null, null),
            CancellationToken.None);

        result.IsT0.ShouldBeTrue();
        result.AsT0.Label.ShouldBe("New Label");
    }
    ...
    public async Task Handle_ShouldReturnNotFoundError_WhenMindMapDoesNotExist()
    {
        ...
        var result = await _handler.Handle(
            new UpdateNodeCommand(2026, Guid.NewGuid(), "Label", null, null, null, null, null, null),
            CancellationToken.None);
