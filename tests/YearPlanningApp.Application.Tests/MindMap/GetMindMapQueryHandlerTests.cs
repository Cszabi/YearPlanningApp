...
public class GetMindMapQueryHandlerTests
{
    private readonly IUnitOfWork _uow = Substitute.For<IUnitOfWork>();
    private readonly ICurrentUserService _currentUser = Substitute.For<ICurrentUserService>();
    private readonly Guid _userId = Guid.NewGuid();
    private readonly GetMindMapQueryHandler _handler;

    public GetMindMapQueryHandlerTests()
    {
        _currentUser.UserId.Returns(_userId);
        _handler = new GetMindMapQueryHandler(_uow, _currentUser);
    }

    [Fact]
    public async Task Handle_ShouldReturnMindMap_WhenItExists()
    {
        var map = new Domain.Entities.MindMap
        {
            ...
            {
                new() { Id = Guid.NewGuid(), Label = "Root", NodeType = MindMapNodeType.Root, PositionX = 0, PositionY = 0 }
            }
        };
        _uow.MindMaps.GetByUserAndYearAsync(_userId, 2026, Arg.Any<CancellationToken>()).Returns(map);

        var result = await _handler.Handle(new GetMindMapQuery(2026), CancellationToken.None);

        result.IsT0.ShouldBeTrue();
        result.AsT0.Year.ShouldBe(2026);
        result.AsT0.Nodes.Count.ShouldBe(1);
    }
